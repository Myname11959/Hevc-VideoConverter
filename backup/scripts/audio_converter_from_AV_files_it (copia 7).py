...
class AudioConverter(QDialog):
    ...
    def _build_ui(self):
        """
        UI piatta a coordinate, ancorata a sinistra.
        - Prima riga: "Input track:" + path (QLineEdit)
        - "Traccia:" usa stessa larghezza del path
        - Denoise nr prima di Gain (dB)
        - EQ su una riga; Reverb/Stereo/Compr su una riga
        - Mantieni MONO + Evita clipping su una riga
        - Preview su una riga
        - Textarea ffmpeg 2 righe (larghezza regolabile)
        - Profilo soundbar + 2 Samsung sulla stessa riga
        - Footer con bottoni uniformi
        """
        from hevc_gui.core import constants as C
        from PyQt5.QtWidgets import (
            QVBoxLayout,
        ...
        # ---- finestra e contenitore ---------------------------------------------
        self.setWindowTitle("Audio Converter")
        self.resize(M["WIN_W"], 620)
        self.setFixedWidth(M["WIN_W"])
        ...
        # ===== Manopole globali ===================================================
        W_TRACK_REQ = 470  # larghezza desiderata per Path + combo "Traccia"
        W_LIST = 520  # larghezza area ffmpeg (textarea)
        LIST_ROWS = 2  # righe visibili nella textarea

        # Clamp per larghezza TRACCIA/PATH rispetto allo spazio reale accanto a "Traccia:"
        fm = self.fontMetrics()
        ...
        self.path.setMinimumWidth(W_TRACK)
        self.path.setMaximumWidth(W_TRACK)
        ...
        # -- R1: Tratta come muto --------------------------------------------------
        x = M["X0"]
        self.chk_force_mute = lone(QCheckBox("Tratta come muto", self), 180)
        new_line()
        ...
        # -- R6: Reverb / Stereo Enh / Compr in una riga --------------------------
        x = M["X0"]
        ...
        x = M["X0"]
        self.chk_dyn = lone(QCheckBox("Auto-loudness (DynAudNorm)", self), 250)
        self.chk_dialog_boost = lone(QCheckBox("Dialog Boost (+2 dB @ 2 kHz)", self), 270)
        new_line()
        ...
        # -- R9: Preview (tutto su una riga) --------------------------------------
        x = M["X0"]
        prev_lab = QLabel("Preview:", self)
        prev_lab.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        ...
        self.btn_prev.setFixedHeight(M["H_BTN"])
        lone(self.btn_prev, M["W_BTN"])
        new_line()

        # -- R10: Textarea comandi (2 righe) ---------------------------------------
        x = M["X0"]
        ...
        BTN_GAP = 8  # distanza tra i pulsanti
        BTN_SHIFT_LEFT = 60  # sposta il gruppo verso sinistra (0 = tutto a destra)
        ...
        w_add, w_cancel, w_ok = fit_btn(self.btn_add.text()), fit_btn(self.btn_cancel.text()), fit_btn(self.btn_ok.text())

        total_btn_w = w_add + w_cancel + w_ok + 2 * BTN_GAP
        x_btn = CANVAS_W - total_btn_w - BTN_SHIFT_LEFT  # per ancorarli a sinistra: x_btn = M["X0"]

        place(self.btn_add, x_btn, w_add, M["H_BTN"])
        place(self.btn_cancel, x_btn + w_add + BTN_GAP, w_cancel, M["H_BTN"])
        place(self.btn_ok, x_btn + w_add + BTN_GAP + w_cancel + BTN_GAP, w_ok, M["H_BTN"])

        # ---- dimensionamento finale ---------------------------------------------
        final_h = y + M["ROW_H"] + 12
        self.canvas.setFixedHeight(final_h)
        total_h = M["MARG"] * 2 + path_row.sizeHint().height() + 6 + final_h
        self.setFixedHeight(min(max(total_h, 520), 720))

        # ---- combo sizing (anti-troncatura) --------------------------------------
