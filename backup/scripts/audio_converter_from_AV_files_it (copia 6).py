...
class AudioConverter(QDialog):
    ...
    def _build_ui(self):
        ...
        # === Misure finestra e blocco centrale ===
        ...
        def _gap(px=10):
            lab = QLabel(" ", self)
            lab.setFixedWidth(px)
            lab.setObjectName("gap")
            return lab
        ...
        add_row_centered(lbl_br, self.cmb_br, _gap(), lbl_sr, self.cmb_sr, stretch_last=True)
        ...
        row5_box, row5_layout = add_row_centered(
            self.chk_nr, _gap(8), lbl_nr, self.in_nr, _gap(8), lbl_gain, self.cmb_gain, stretch_last=True
        )
        ...
        row6_box, row6_layout = add_row_centered(
            lbl_b, self.cmb_eq_bass, _gap(), lbl_m, self.cmb_eq_mid, _gap(), lbl_h, self.cmb_eq_treb, stretch_last=True
        )

        # ---------------- RIGA 7: Reverb → Stereo Enh → Compr. ----------------
        ...
        row7_box, row7_layout = add_row_centered(
            lbl_rev, self.cmb_rev, _gap(6), lbl_st, self.cmb_stereo, _gap(6), lbl_comp, self.cmb_comp_soft, stretch_last=True
        )

        # ---------------- CALIBRAZIONE: larghezze/spacing (solo dove serve) ----------------
        try:
            ...
            # Spaziatura compatta
            for lay in (row5_layout, row6_layout):
                lay.setSpacing(4)
            row7_layout.setSpacing(2)  # Reverb/Stereo più vicine alla loro label

            # NUMERICI compatti
            NUM_W = fm.horizontalAdvance("-18") + 26
            for w in (self.cmb_eq_bass, self.cmb_eq_mid):
                w.setMinimumWidth(NUM_W)
                w.setMaximumWidth(NUM_W)

            # === Allineamenti orizzontali di riferimento ===
            high_w = self.cmb_eq_treb.sizeHint().width()
            # Gain = High (bordo destro identico)
            self.cmb_gain.setMinimumWidth(high_w)
            self.cmb_gain.setMaximumWidth(high_w)
            # High resta com’è
            self.cmb_eq_treb.setMinimumWidth(high_w)
            self.cmb_eq_treb.setMaximumWidth(high_w)

            # --- Terza riga (SOLO queste tre combo) ---
            # Compr. = High/Gain (bordo destro uguale)
            self.cmb_comp_soft.setMinimumWidth(high_w)
            self.cmb_comp_soft.setMaximumWidth(high_w)

            # Reverb e Stereo Enh.: accorciate da destra, niente tagli
            rev_min = max(110, self.cmb_rev.sizeHint().width() - 12)
            st_min = max(110, self.cmb_stereo.sizeHint().width() - 10)
            self.cmb_rev.setMinimumWidth(rev_min)
            self.cmb_rev.setMaximumWidth(rev_min)
            self.cmb_stereo.setMinimumWidth(st_min)
            self.cmb_stereo.setMaximumWidth(st_min)

            # Policy per non mozzare frecce/testi
            for w in (
                self.in_nr,
                self.cmb_eq_bass,
                self.cmb_eq_mid,
            ...
            # Denoise edit: dimensione giusta
            self.in_nr.setMinimumWidth(max(self.in_nr.sizeHint().width(), fm.horizontalAdvance("0–30 dB") + 14))
            self.in_nr.setMaximumWidth(self.in_nr.minimumWidth())
        ...
        add_row_centered(self.chk_dyn, _gap(), self.chk_dialog_boost, stretch_last=True)
        ...
        add_row_centered(self.chk_keep_mono, _gap(), self.chk_anticlip, stretch_last=True)
        ...
        add_row_centered(lbl_prev, lbl_start, self.te_prev_start, _gap(), lbl_dur, self.cmb_prev, _gap(), self.btn_prev, stretch_last=True)

        # ---------------- RIGA 11: lista comandi (~2 righe) ----------------
        ...
        add_row_centered(self.chk_sb_stereo, _gap(), self.chk_sb_51, stretch_last=True)
        self._soundbar_injected = True

        # ---------------- FOOTER: placeholder a sx + bottoni a dx ----------------
        ...
        vmain.addWidget(footer_box)

        # ---------------- Selettore di profilo (mutua esclusione + regole) ----------------
        self._active_profile = "none"
        self._stereo_enh_saved = None

        def _restore_stereo_enh_if_needed():
            if self._stereo_enh_saved is not None:
                idx, was_enabled = self._stereo_enh_saved
                try:
                    if 0 <= idx < self.cmb_stereo.count():
                        self.cmb_stereo.setCurrentIndex(idx)
                    self.cmb_stereo.setEnabled(was_enabled)
                except Exception:
                    pass
                self._stereo_enh_saved = None

        def _disable_stereo_enh_for_51():
            if self._stereo_enh_saved is None:
                self._stereo_enh_saved = (self.cmb_stereo.currentIndex(), self.cmb_stereo.isEnabled())
            try:
                idx_none = -1
                for i in range(self.cmb_stereo.count()):
                    if self.cmb_stereo.itemText(i).strip().lower() == "nessuno":
                        idx_none = i
                        break
                self.cmb_stereo.setCurrentIndex(idx_none if idx_none >= 0 else 0)
                self.cmb_stereo.setEnabled(False)
            except Exception:
                pass

        def _select_profile(which: str):
            for cb, want in (
                (self.chk_force_stereo, which == "downmix"),
                (self.chk_sb_stereo, which == "sb_tvj"),
                (self.chk_sb_51, which == "sb_51"),
            ):
                try:
            ...
            if which == "sb_tvj":
                ...
                self._active_profile = which
                self._soundbar_profile = "none" if which != "sb_tvj" else "samsung_stereo"
            if which == "sb_51":
                _disable_stereo_enh_for_51()
            else:
                _restore_stereo_enh_if_needed()
            try:
                self._update_pan_preset_label()
            except Exception:
                pass
        ...
        def _on_force_stereo(ticked: bool):
            if ticked:
                _select_profile("downmix")
            else:
                if not (self.chk_sb_stereo.isChecked() or self.chk_sb_51.isChecked()):
                    _select_profile("none")

        def _on_sb_stereo(ticked: bool):
            if ticked:
                _select_profile("sb_tvj")
            else:
                if not (self.chk_force_stereo.isChecked() or self.chk_sb_51.isChecked()):
                    _select_profile("none")

        def _on_sb_51(ticked: bool):
            if ticked:
                _select_profile("sb_51")
            else:
                if not (self.chk_force_stereo.isChecked() or self.chk_sb_stereo.isChecked()):
                    _select_profile("none")
        ...
        self.chk_force_stereo.toggled.connect(_on_force_stereo)
        self.chk_sb_stereo.toggled.connect(_on_sb_stereo)
        self.chk_sb_51.toggled.connect(_on_sb_51)
