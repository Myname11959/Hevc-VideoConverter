...
from .menubar import setup_menubar, refresh_icons
...
# Logger
# —————————————————————————————————————————————————————————————
log_dir = Path(SCRIPTS_DIR) / "log"
log_dir.mkdir(parents=True, exist_ok=True)
log_path = log_dir / "gui_debug.log"
if log_path.exists():
    log_path.unlink()
...
logger.debug("=== Avvio applicazione: log ripulito ===")
...
class MainWindow(QMainWindow):
    ...
    def build_ffmpeg_external_audio_cmd(
        ...
        # garantisci input corretto e no video
        if "-i" not in spec:
        ...
        # se manca -af, inietta catena "alla copia": SOXR + dynaudnorm
        if "-af" not in spec:
            spec += ["-af", "aresample=resampler=soxr,dynaudnorm=f=250:g=31:p=0.95:m=50"]

        # completa command
        cmd += spec + container + tail

        # limiti leggeri come nei “copia” (non obbligatori per la sync, ma utili)
    ...
    def build_ffmpeg_audio_cmds(
        ...
        for i, spec in enumerate(self._audio_opts):
            ...
            # output name/estensione a seconda del codec (default AAC → .m4a + ipod)
            def _get_flag(seq, keys, default=None):
                for k in keys:
                    if k in seq:
                        p = seq.index(k)
            ...
            while True:
                ...
                if tok == "-map":
                    ...
                    # ignora tutto il resto (flag non audio o duplicati)
                    pass
    ...
    @pyqtSlot()
    def show_info(self):
        # dimensioni “modificabili” logo
        logo_w, logo_h = 140, 140

        # creo un QDialog personalizzato
        dlg = QDialog(self)
        dlg.setWindowTitle("Info")
        layout = QVBoxLayout(dlg)

        # 1) icona centrata in cima
