...
SINK_NAMES = {
    ...
    "setPlaceholderText",
    # constructors commonly used with text
    "QCheckBox", "QPushButton", "QLabel", "QGroupBox",
}

TECH_PATTERNS = [
    re.compile(r"^:/"),                 # Qt resource paths
    re.compile(r"/"),                   # any path-like (icons, folders)
    re.compile(r"^\.[A-Za-z0-9]{2,5}$"),# extensions like .vob .ifo
...
def should_translate(s: str) -> bool:
    ...
    if "ffmpeg" in t.lower():
        return False
    # almeno una lettera (evita roba tipo "1", ":" ecc.)
...
def main() -> int:
    ...
    backup_dir = bdir or Path("/tmp/i18n_ldvd_gui_sinks_bak")
